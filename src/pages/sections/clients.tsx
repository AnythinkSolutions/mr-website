...
function ClientsSection({data}: {data: IClient[]}){
  ...
  const magazineClients = useMemo<IClient[]>(() => data?.filter(c => !c.isHidden && c.category === "magazine").slice(0, 14) as IClient[], [data]);
  const contentClients = useMemo<IClient[]>(() => data?.filter(c => !c.isHidden && c.category === "content").slice(0, 14) as IClient[], [data]);
